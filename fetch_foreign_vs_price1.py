...
def fetch_invest_count(dates):
    """針對每個日期呼叫 T86 JSON 介面，擷取投信買賣超『張數』。"""
    recs = []
    API = "https://www.twse.com.tw/fund/T86?response=json&date={}&selectType=ALL"
    for d in dates:
        try:
        ...
        if '證券代號' not in fields:
            continue
        idx_code = fields.index('證券代號')
        idx_invest = next((i for i, col in enumerate(fields) if '投信' in col and '買賣超' in col), None)
        if idx_invest is None:
            print(f"⚠️ {d} 無法找到投信買賣超欄位，fields: {fields}")
            continue
        for row in data:
            code = str(row[idx_code]).strip('=" ')
            if code == STOCK_NO:
                val = int(str(row[idx_invest]).replace(',', '').strip()) // 1000
                recs.append({'date': pd.to_datetime(d, format='%Y%m%d'),
                             '投信買賣超張數': val})
                break
        time.sleep(0.1)
...
def main():
    ...
    df_i = fetch_invest_count(dates)
    if df_i.empty:
        raise RuntimeError("❌ 未取得任何投信買賣超資料。")

    print("✅ 外資資料日期：", df_f.index.tolist())
    print("✅ 投信資料日期：", df_i.index.tolist())

    df = pd.merge(df_f, df_i, left_index=True, right_index=True, how='outer').sort_index()
    df = df.fillna(0)

    # 繪製柱狀圖
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.bar(df.index, df['外資買賣超張數'], label='外資買賣超', color='blue', alpha=0.6)
    ax.bar(df.index, df['投信買賣超張數'], label='投信買賣超', color='green', alpha=0.6)
    ax.set_ylabel('買賣超張數', color='black')
    ax.set_title(f"{STOCK_NO} 外資與投信買賣超張數（近 {DAYS} 交易日）")
